...
Package, command, and import name: **specdrive**.
...
- [ ] Bump `SCHEMA_VERSION` in `src/specdrive/state.py` if releasing a new version
...
Confirm `playbook.md` ships inside the wheel (adapters and `specdrive playbook`
depend on it):
